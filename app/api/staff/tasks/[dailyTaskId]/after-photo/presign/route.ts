...
export async function POST(req: Request, ctx: { params: Promise<{ dailyTaskId: string }> }) {
  ...
  const rows = await prisma.$queryRaw<
    Array<{ id: number; staffId: number; status: string; beforePhotoUrl: string | null }>
  >(
    Prisma.sql`SELECT id, "staffId", "status", "beforePhotoUrl" FROM "DailyStaffTask" WHERE id = ${id} LIMIT 1`
  );
  const task = rows[0];
