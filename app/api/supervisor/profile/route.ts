...
import { buildContextLabel } from "@/lib/supervisor-dashboard";
...
export async function GET(req: Request) {
  ...
  if (auth.role !== ROLE_NAMES.SUPERVISOR) {
  ...
  const [user, attendance, contextLabel] = await Promise.all([
    ...
    prisma.staffAttendance.findUnique({
      where: {
        staffId_workDate: { staffId: supervisorId, workDate: today }
      }
    }),
    buildContextLabel(supervisorId, today)
  ]);
