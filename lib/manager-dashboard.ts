...
/**
 * Context tag: distinct property names from today's managed team daily tasks, then shift.
 * Multiple properties joined with " · ".
 */
export async function buildContextLabelForManager(managerId: number, taskDate: Date): Promise<string> {
...
export async function getTeamApprovalKpis(managerId: number, taskDate: Date) {
...
export async function getTeamShiftCompletion(managerId: number, taskDate: Date) {
...
export async function getZoneHealthForManager(managerId: number, taskDate: Date): Promise<ZoneHealthRow[]> {
  const rows = await prisma.$queryRaw<
    Array<{
      zoneId: number;
      zoneName: string;
...
function isTaskDone(status: string, approvalStatus: string | null): boolean {
...
export async function getUrgentTasksForManager(
  managerId: number,
  taskDate: Date,
  now: Date,
  limit: number
...
export async function getRecentActivityForManager(managerId: number, limit: number): Promise<RecentActivityRow[]> {
  const rows = await prisma.$queryRaw<
    Array<{
      id: number;
      status: string;
...
export async function getManagerDashboardData(managerId: number, taskDate: Date, now: Date) {
  const [user, contextLabel, kpis, completion, zones, urgent, recent] = await Promise.all([
    prisma.user.findUnique({
      where: { id: managerId },
      select: { name: true, email: true }
    }),
    buildContextLabelForManager(managerId, taskDate),
    getTeamApprovalKpis(managerId, taskDate),
    getTeamShiftCompletion(managerId, taskDate),
    getZoneHealthForManager(managerId, taskDate),
    getUrgentTasksForManager(managerId, taskDate, now, 20),
    getRecentActivityForManager(managerId, 20)
  ]);
