...
export async function getShiftOverview(supervisorId: number, taskDate: Date): Promise<ShiftReportOverview> {
...
export async function getZoneCompletion(supervisorId: number, taskDate: Date): Promise<ZoneReportRow[]> {
...
export async function getFunctionHealth(
  ...
  // Ensure stable ordering by group list, then append unknown keys (shouldn’t happen)
...
export async function getEmployeePerformance(supervisorId: number, taskDate: Date): Promise<EmployeePerformanceRow[]> {
...
export async function getEmployeeShiftLogs(
  supervisorId: number,
  staffId: number,
  taskDate: Date
  ...
  // Ensure staff belongs to supervisor
  const staff = await prisma.user.findFirst({
    where: { id: staffId, supervisorId },
  ...
  const [summaryAll, rows] = await Promise.all([
    getEmployeePerformance(supervisorId, taskDate),
    prisma.$queryRaw<
      Array<{
        dailyTaskId: number;
        taskTitle: string;
...
export async function getEscalations(supervisorId: number, taskDate: Date, now: Date): Promise<EscalationRow[]> {
  ...
  const [attendanceRows, taskRows] = await Promise.all([
    prisma.$queryRaw<Array<{ staffId: number; staffName: string; status: string | null }>>(
      Prisma.sql`
        ...
        WHERE u."supervisorId" = ${supervisorId}
        ORDER BY u."name" ASC
      `
    ),
    prisma.$queryRaw<
  ...
  // NO_SHOW: ABSENT or missing attendance row
...
export async function getSupervisorShiftReport(
  ...
  const [overview, zones, functions, employees, escalations] = await Promise.all([
    getShiftOverview(supervisorId, d),
    getZoneCompletion(supervisorId, d),
    getFunctionHealth(supervisorId, d, groups),
    getEmployeePerformance(supervisorId, d),
    getEscalations(supervisorId, d, now)
  ]);
